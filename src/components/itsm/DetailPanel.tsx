...
export default function DetailPanel({ state, results }: Props) {
  const rows = [
    ...
    { label: "Chamados de Firewall", value: formatNumber(results.chamadosSistemas), desc: `${formatNumber(state.qtdSistemas)} × ${state.taxaSistemas}` },
    { label: "", value: "", desc: "", separator: true },
    ...
    { label: "  ↳ de Firewall no N3", value: formatNumber(results.chamadosSistemas * (state.percN3 / 100), 1), desc: `${formatNumber(results.chamadosSistemas)} × ${state.percN3}%` },
    { label: "", value: "", desc: "", separator: true },
