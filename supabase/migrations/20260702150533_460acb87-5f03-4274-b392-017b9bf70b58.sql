...
-- Lock down internal queue helpers: revoke public/anon/authenticated EXECUTE
-- and pin search_path on the SECURITY DEFINER functions.
...
-- Rewrite queue helpers to use fully-qualified pgmq functions (since search_path is now empty).
...
-- Also pin trigger helpers' search_path (touch_updated_at/touch_atualizado_em already set).
-- Allow approval role members to read their own membership rows.
